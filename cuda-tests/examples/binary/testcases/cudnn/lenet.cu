...
size_t ReadUByteDataset(const char* image_filename, const char* label_filename, 
    ...
    assert(!data || fread(data, sizeof(*data), length * w * h, fp) == length * w * h);
    fclose(fp);
    ...
    assert(!labels || fread(labels, sizeof(*labels), length, fp) == length);
...
// Batch parameters
DEFINE_uint64(batch_size, 128, "Batch size for training");
...
DEFINE_string(train_images, "/tmp/train-images-idx3-ubyte", "Training images filename");
DEFINE_string(train_labels, "/tmp/train-labels-idx1-ubyte", "Training labels filename");
DEFINE_string(test_images, "/tmp/t10k-images-idx3-ubyte", "Test images filename");
DEFINE_string(test_labels, "/tmp/t10k-labels-idx1-ubyte", "Test labels filename");
...
/* __global__ void SoftmaxLossBackprop(const float *label, int num_labels, int batch_size, float *diff)
{
    ...
    // printf("idx = %d, label = %d, num_labels = %d, diff = %f\n", idx, label_value, num_labels, diff[idx * num_labels + label_value]);
...
struct TrainingContext
{
                         ...
                         void *workspace, float *onevec)
    {
        ...
        // int blocks = 1024, cycle = (m_batchSize + blocks - 1) / blocks;
        // Softmax layer
        // SoftmaxLossBackprop<<<cycle, blocks>>>(labels, ref_fc2.outputs, m_batchSize, dloss_data);
        
        checkCudaErrors(cublasSaxpy(cublasHandle, ref_fc2.outputs * m_batchSize, &alpha, labels, 1, dloss_data, 1));
...
{
    system("curl -L http://yann.lecun.com/exdb/mnist/train-images-idx3-ubyte.gz | gunzip > /tmp/train-images-idx3-ubyte");
    system("curl -L http://yann.lecun.com/exdb/mnist/train-labels-idx1-ubyte.gz | gunzip > /tmp/train-labels-idx1-ubyte");
    system("curl -L http://yann.lecun.com/exdb/mnist/t10k-images-idx3-ubyte.gz | gunzip > /tmp/t10k-images-idx3-ubyte");
    system("curl -L http://yann.lecun.com/exdb/mnist/t10k-labels-idx1-ubyte.gz | gunzip > /tmp/t10k-labels-idx1-ubyte");
    ...
    */
    ...
    checkCudaErrors(cudaMalloc(&d_labels,  sizeof(float) * context.m_batchSize * fc2.outputs));
    ...
    float one = 1.0f;
    checkCudaErrors(cudaMemset(d_onevec, *(int*)&one, sizeof(float) * context.m_batchSize));
    
    printf("Preparing dataset\n");
    
    // Normalize training set to be in [0,1]
    std::vector<float> train_images_float(train_images.size()), train_labels_float(train_size * fc2.outputs);
    for (size_t i = 0; i < train_size * channels * width * height; ++i)
        train_images_float[i] = (float)train_images[i] / 255.0f;
    
    for (size_t i = 0; i < train_size; ++i)
        train_labels_float[i * fc2.outputs + train_labels[i]] = -1.0f;
    ...
    for (int iter = 0; iter < FLAGS_iterations; ++iter)
    {
        ...
        checkCudaErrors(cudaMemcpyAsync(d_labels, &train_labels_float[imageid * context.m_batchSize * fc2.outputs],
                                        sizeof(float) * context.m_batchSize * fc2.outputs, cudaMemcpyHostToDevice));
    ...
    if (classifications > 0)
    {
        ...
        assert(classification_error < 0.2);
    }
