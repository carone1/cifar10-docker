...
def read_data(label, image):
    ...
    with gzip.open(download_file(base_url+label, os.path.join('/tmp/data',label))) as flbl:
        magic, num = struct.unpack(">II", flbl.read(8))
        label = np.fromstring(flbl.read(), dtype=np.int8)
    with gzip.open(download_file(base_url+image, os.path.join('/tmp/data',image)), 'rb') as fimg:
