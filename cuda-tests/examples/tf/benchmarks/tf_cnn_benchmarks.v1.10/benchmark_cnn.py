...
def create_config_proto(params):
  ...
  if params.variable_update == 'horovod':
    ...
    # Set Device Visibility to Avoid Useless many-vGPU connection
    import os
    if 'ASAKA_MODE' in os.environ:
      os.environ['ASAKA_VISIBLE_DEVICES'] = str(hvd.local_rank())
    else:
...
class BenchmarkCNN(object):
  ...
  def add_forward_pass_and_gradients(self,
    ...
    print('Create model replias on device `%s` ..' % labels.device)

    if 'DATA_URL' in os.environ:
      with tf.device(self.devices[rel_device_num]):
        from tensorflow.contrib import image_pipe
        images, labels = image_pipe.flow_from_directory(
		directory_url=os.environ['DATA_URL'],
		image_format='NHWC',
