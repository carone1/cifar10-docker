...
from official.resnet import resnet_run_loop_distributed
...
def input_fn(is_training,
  ...
  return resnet_run_loop_distributed.process_record_dataset(
      dataset=dataset,
...
def get_synth_input_fn(dtype):
  return resnet_run_loop_distributed.get_synth_input_fn(
...
def cifar10_model_fn(features, labels, mode, params):
  ...
  learning_rate_fn = resnet_run_loop_distributed.learning_rate_with_decay(
  ...
  return resnet_run_loop_distributed.resnet_model_fn(
      features=features,
      labels=labels,
      mode=mode,
...
def define_cifar_flags():
  resnet_run_loop_distributed.define_resnet_flags()
  flags.adopt_module_key_flags(resnet_run_loop_distributed)
...
def run_cifar(flags_obj):
  ...
  input_function = (flags_obj.use_synthetic_data and
                    ...
                    input_fn)
  result = resnet_run_loop_distributed.resnet_main(
...
def get_tf_cluster_config():
  ...
  parser.add_argument(
      ...
      type=int,
      default=0,
      help="Number of GPUs used on the server"
  )
